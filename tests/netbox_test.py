...
from tests.mock_spectrum_device import mock_spectrum_netbox_factory, NUM_CHANNELS_IN_MOCK_MODULE, \
    NUM_MODULES_IN_MOCK_DEVICE, NUM_DEVICES_IN_MOCK_NETBOX
from tests.single_device_test import SingleDeviceTest
from third_party.specde.py_header.regs import CHANNEL0, CHANNEL3, CHANNEL6, CHANNEL9, CHANNEL12, CHANNEL15, CHANNEL2, \
    CHANNEL4, SPC_CHENABLE, CHANNEL1, CHANNEL5, CHANNEL7


class NetboxTest(SingleDeviceTest):

    def setUp(self) -> None:
        self._device = mock_spectrum_netbox_factory()
    ...
    def test_get_channels(self) -> None:
        channels = self._device.channels
        expected_channels = [SpectrumChannel(SpectrumChannelName.CHANNEL0, self._device._devices[0]),
