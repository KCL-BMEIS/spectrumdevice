...
class MockSpectrumCard(SpectrumCard, MockSpectrumDevice):
    def __init__(self, source_frame_rate_hz: float = 10.0):
        ...
        Args:
            source_frame_rate_hz (float): Frame rate at which the mock waveform source will generate waveforms.
...
class MockSpectrumStarHub(SpectrumStarHub, MockSpectrumDevice):
    def __init__(
        self, hub_handle: DEVICE_HANDLE_TYPE, child_cards: Sequence[SpectrumCard], master_card_index: int
    ) -> None:
        """MockSpectrumStarHub
        ...
        Args:
            source_frame_rate_hz (float): Frame rate at which the mock waveform sources will generate waveforms.
