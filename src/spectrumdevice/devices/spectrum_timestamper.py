...
class Timestamper(ABC):
    ...
    def _mark_transfer_buffer_elements_as_free(self, num_available_bytes: int) -> None:
        self._parent_device.write_to_spectrum_device_register(
            SPC_TS_AVAIL_CARD_LEN, num_available_bytes
        )
    ...
    def get_timestamp(self) -> datetime:
        ...
        if (
            len(trigger_source) == 1
            and trigger_source[0] == TriggerSource.SPC_TMASK_SOFTWARE
        ):
            return datetime.now()
        ...
        while (n_kept_bytes < self._expected_timestamp_bytes_per_frame) and (
            poll_count < MAX_POLL_COUNT
        ):

            n_bytes_not_yet_received = (
                self._expected_timestamp_bytes_per_frame - n_kept_bytes
            )
            num_available_bytes = self._parent_device.read_spectrum_device_register(
                SPC_TS_AVAIL_USER_LEN
            )
            start_pos_int_bytes = self._parent_device.read_spectrum_device_register(
                SPC_TS_AVAIL_USER_POS
            )

            # don't go beyond the size of the ts data buffer
            if (
                start_pos_int_bytes + num_available_bytes
                ...
                kept_bytes += list(
                    copy(
                        self._transfer_buffer.data_array[
                            start_pos_int_bytes : start_pos_int_bytes + n_bytes_to_keep
                        ]
                    )
                )
        ...
        timestamp_in_samples = struct.unpack(
            "<2Q", struct.pack(f"<{len(kept_bytes)}B", *kept_bytes)
        )[0]
