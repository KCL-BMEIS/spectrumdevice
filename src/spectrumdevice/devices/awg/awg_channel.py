...
class SpectrumAWGIOLine(AbstractSpectrumIOLine, SpectrumAWGIOLineInterface):
    ...
    def _get_io_line_mode_settings_mask(self, mode: IOLineMode) -> int:
        if mode == IOLineMode.SPCM_XMODE_DIGOUT:
            return (
                self._dig_out_settings.source_channel.value
                | self._dig_out_settings.source_bit.value
            )
        else:
            return 0


class SpectrumAWGAnalogChannel(
    AbstractSpectrumAnalogChannel, SpectrumAWGAnalogChannelInterface
):
    ...
    def _set_settings_from_dict(self, settings: dict) -> None:
        self.set_signal_amplitude_in_mv(
            settings[SpectrumAWGAnalogChannel.signal_amplitude_in_mv.__name__]
        )
        self.set_dc_offset_in_mv(
            settings[SpectrumAWGAnalogChannel.dc_offset_in_mv.__name__]
        )
        self.set_output_filter(
            settings[SpectrumAWGAnalogChannel.output_filter.__name__]
        )
        self.set_stop_level_mode(
            settings[SpectrumAWGAnalogChannel.stop_level_mode.__name__]
        )
        self.set_stop_level_custom_value(
            settings[SpectrumAWGAnalogChannel.stop_level_custom_value.__name__]
        )
    ...
    def is_switched_on(self) -> bool:
        ...
        return bool(
            self._parent_device.read_spectrum_device_register(
                OUTPUT_CHANNEL_ENABLED_COMMANDS[self._number]
            )
        )
    ...
    def set_dc_offset_in_mv(self, dc_offset: int) -> None:
        ...
        self._parent_device.write_to_spectrum_device_register(
            OUTPUT_DC_OFFSET_COMMANDS[self._number], dc_offset
        )
    ...
    def set_signal_amplitude_in_mv(self, amplitude: int) -> None:
        ...
        self._parent_device.write_to_spectrum_device_register(
            OUTPUT_AMPLITUDE_COMMANDS[self._number], amplitude
        )
    ...
    def set_output_filter(self, output_filter: OutputChannelFilter) -> None:
        self._parent_device.write_to_spectrum_device_register(
            OUTPUT_FILTER_COMMANDS[self._number], output_filter.value
        )
    ...
    def stop_level_mode(self) -> OutputChannelStopLevelMode:
        ...
        return OutputChannelStopLevelMode(
            self._parent_device.read_spectrum_device_register(
                OUTPUT_STOP_LEVEL_MODE_COMMANDS[self._number]
            )
        )

    def set_stop_level_mode(self, mode: OutputChannelStopLevelMode) -> None:
        self._parent_device.write_to_spectrum_device_register(
            OUTPUT_STOP_LEVEL_MODE_COMMANDS[self._number], mode.value
        )
    ...
    def stop_level_custom_value(self) -> int16:
        ...
        return int16(
            self._parent_device.read_spectrum_device_register(
                OUTPUT_STOP_LEVEL_CUSTOM_VALUE_COMMANDS[self._number]
            )
        )
